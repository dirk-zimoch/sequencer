...
void generate_code(Node *prog)
{
	...
	traverse_syntax_tree(prog, bit(E_VAR), 0, 0, assert_var_declared, 0);
	...
	/* Initial comments */
	gen_code("/* C code for program %s, ", prog->token.str);
	...
	foreach (defn, prog->prog_defns) gen_global_defn(ctx, defn);

	/* Variable declarations */
	gen_var_struct(prog, p->options->reent);

	/* Function declarations */
	gen_func_decls(prog);

	/* State and state set functions */
	gen_ss_code(ctx, prog, p->chan_list, p->evflag_list);

	/* Channel, state set, and program tables */
	gen_tables(prog);

	/* Extra definitions */
	foreach (defn, prog->prog_xdefns) gen_global_defn(ctx, defn);

	/* Main function */
	if (p->options->main) gen_main(prog->token.str);

	/* Sequencer registration */
	gen_init_reg(prog->token.str);
}
